...
// ── RS-16 constants (same sensor as ground robot) ────────────────────────────
...
static constexpr double RANGE_NOISE_STD = 0.02;
static constexpr double DROPOUT_PROB    = 0.02;
static constexpr double LIDAR_OFFSET_Z  = 0.15;  // lidar above drone CoM (m)
...
class DroneLidarNode : public rclcpp::Node
{
public:
  DroneLidarNode()
  : Node("drone_lidar_node"),
    noise_gen_(std::random_device{}()),
    noise_(0.0, RANGE_NOISE_STD),
    drop_dist_(0.0, 1.0)
  {
    // All topic names are relative — namespace applied by launch file
    pose_sub_  = create_subscription<geometry_msgs::msg::PoseStamped>(
      "estimated_pose", 10,
      std::bind(&DroneLidarNode::poseCallback, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(get_logger(),
      "Drone RS-16 lidar started: %d ch × %d pts, lidar offset +%.2f m above drone",
      NUM_CHANNELS, NUM_HORIZONTAL, LIDAR_OFFSET_Z);
  }

private:
  // ── World geometry — identical to ground robot simulator ──────────────────

  struct Segment { double x1, y1, x2, y2, max_z; };
  struct TopFace { double cx, cy, hw, hd, z; };
  struct Obstacle { double cx, cy, hw, hd, height; float r, g, b; };
  ...
  // Rolling-shutter pose history (x, y, z, theta)
  double prev_x_{0}, prev_y_{0}, prev_z_{0}, prev_theta_{0};
  double curr_x_{0}, curr_y_{0}, curr_z_{0}, curr_theta_{0};
  ...
  // ── Build world (same deterministic layout as lidar_node.cpp) ─────────────
  ...
  void buildWorld()
  {
    ...
    std::mt19937 rng(42);  // same seed — same world as ground simulator
    ...
    for (int i = 0; i < 25; ) {
      ...
      if (cx * cx + cy * cy < 16.0) continue;
      double hw = half_sz(rng), hd = half_sz(rng), height = ht_dist(rng);
  ...
  // ── Ray casting (same physics as lidar_node.cpp) ──────────────────────────
                   ...
                   double dx, double dy, double dz) const
  {
    ...
    for (const auto & s : world_) {
      ...
      if (z_hit < 0.0) continue;
      if (s.max_z > 0.0 && z_hit > s.max_z) continue;
      min_t = t;
    }
  ...
  {
    prev_x_     = curr_x_;     prev_y_     = curr_y_;
    prev_z_     = curr_z_;     prev_theta_ = curr_theta_;
  ...
  void interpPose(double t,
                  double & x, double & y, double & z, double & theta) const
  {
    x     = prev_x_     + t * (curr_x_     - prev_x_);
    y     = prev_y_     + t * (curr_y_     - prev_y_);
    z     = prev_z_     + t * (curr_z_     - prev_z_);
  ...
  void publishCloud(const rclcpp::Time & stamp)
  {
    ...
    buf.reserve(NUM_CHANNELS * NUM_HORIZONTAL * POINT_STEP / 4);
    uint32_t count = 0;

    for (int h = 0; h < NUM_HORIZONTAL; ++h) {
      double t_frac = static_cast<double>(h) / NUM_HORIZONTAL;
      double rx, ry, rz, rtheta;
      interpPose(t_frac, rx, ry, rz, rtheta);

      double sensor_z = rz + LIDAR_OFFSET_Z;
      ...
      for (int c = 0; c < NUM_CHANNELS; ++c) {
        ...
        // Use live drone altitude as sensor origin height
  ...
  // ── Publish obstacle markers (once at startup) ─────────────────────────────
  ...
  void publishMarkers()
  {
    ...
    for (std::size_t i = 0; i < obstacles_.size(); ++i) {
      ...
      m.pose.position.z    = ob.height * 0.5;
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<DroneLidarNode>());
  rclcpp::shutdown();
  return 0;
}
